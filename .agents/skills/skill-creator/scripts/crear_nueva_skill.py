#!/usr/bin/env python3
"""Crea una skill mínima o regenera el índice canónico del proyecto."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--nombre-skill")
    parser.add_argument("--descripcion-skill")
    parser.add_argument("--condiciones-de-uso")
    args = parser.parse_args()
    creation = (args.nombre_skill, args.descripcion_skill, args.condiciones_de_uso)
    if args.regenerate_index:
        if any(value is not None for value in creation):
            parser.error("--regenerate-index no admite argumentos de creación")
    elif any(value is None for value in creation):
        parser.error("crear una skill requiere nombre, descripción y condiciones de uso")
    return args
...
def build_index(skills_dir: Path) -> str:
    ...
    for skill_md in sorted(skills_dir.glob("*/SKILL.md")):
        ...
        entries.append((name, f".agents/skills/{name}/SKILL.md", description, conditions))
...
def main() -> int:
    ...
    if args.regenerate_index:
        if not skills_dir.is_dir():
            raise FileNotFoundError(f"no se encontró el directorio {skills_dir}")
